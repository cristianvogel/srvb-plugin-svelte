...
(() => {
  ...
  function diffuse(size, ...ins) {
    ...
    (0, import_invariant3.default)(len === 8, "Invalid diffusion step!");
    (0, import_invariant3.default)(typeof size === "number", "Diffusion step size must be a number");
    const dels = ins.map(function(input, i) {
      const lineSize = size * ((i + 1) / len);
      return stdlib.sdelay({ size: lineSize }, input);
    });
    return H8.map(function(row, i) {
      return stdlib.add(...row.map(function(col, j) {
        return stdlib.mul(col * scale, dels[j]);
      }));
    });
  }
  function dampFDN(name, sampleRate, size, decay, modDepth, ...ins) {
    const len = ins.length;
    const scale = Math.sqrt(1 / len);
    const md = stdlib.mul(modDepth, 0.02);
    if (len !== 8)
    ...
    return mix.map(function(mm, i) {
      const modulate = (x, rate, amt) => stdlib.add(x, stdlib.mul(amt, stdlib.cycle(rate)));
      const ms2samps2 = (ms) => sampleRate * (ms / 1e3);
      const delaySize = stdlib.mul(stdlib.add(1, stdlib.mul(3, size)), ms2samps2((i + 1) * 17));
      const readPos = modulate(delaySize, stdlib.add(0.1, stdlib.mul(i, md)), ms2samps2(2.5));
  ...
  function srvb(props, xl, xr) {
    (0, import_invariant3.default)(typeof props === "object", "Unexpected props object");
    const key = props.key;
    const sampleRate = props.sampleRate;
    const size = stdlib.sm(props.size);
    const decay = stdlib.sm(props.decay);
    const modDepth = stdlib.sm(props.mod);
    const mix = stdlib.sm(props.mix);
    const mid = stdlib.mul(0.5, stdlib.add(xl, xr));
    const side = stdlib.mul(0.5, stdlib.sub(xl, xr));
    const four = [xl, xr, mid, side];
    const eight = [...four, ...four.map((x) => stdlib.mul(-1, x))];
    const ms2samps2 = (ms) => sampleRate * (ms / 1e3);
    const d1 = diffuse(ms2samps2(43), ...eight);
    const d2 = diffuse(ms2samps2(97), ...d1);
    const d3 = diffuse(ms2samps2(117), ...d2);
    const d4 = dampFDN(`${key}:d4`, sampleRate, size, 4e-3, modDepth, ...d3);
    const r0 = dampFDN(`${key}:r0`, sampleRate, size, decay, modDepth, ...d4);
    const yl = stdlib.mul(0.25, stdlib.add(r0[0], r0[2], r0[4], r0[6]));
    const yr = stdlib.mul(0.25, stdlib.add(r0[1], r0[3], r0[5], r0[7]));
    return [
      stdlib.select(mix, yl, xl),
      stdlib.select(mix, yr, xr)
    ];
  }

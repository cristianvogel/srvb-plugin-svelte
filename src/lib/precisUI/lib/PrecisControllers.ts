...
export class PrecisUI {
	...
	static getRegistry(): WidgetRegister {
		return get(WidgetStore);
	}
	static getWidgetByTag(tag: string) {
		return get(WidgetStore).get(tag) || undefined;
	}
...
abstract class PrecisController extends PrecisUI {
	...
	getCSSforRect(xywh: Rect): BoundingRectCSS {
		const aRect: BoundingRectCSS = `top:${xywh.y}px;left:${xywh.x}px;width:${xywh.width}px;height:${xywh.height}px;`;
		return aRect;
	}
	/**
	...
	containerTransform(widget: BasicController, scale?: number, newRect?: Rect): string {
		...
		const inline = `${widget.getCSSforRect(rect1)};
...
export class BasicController extends PrecisController {
	...
	getRoundedReadout(): RoundedReadout {
		return roundTo(this.getMappedValue(), this.precis ? 1.0e-4 : 1.0e-2).toFixed(
			this.precis ? 3 : 1
		);
	}

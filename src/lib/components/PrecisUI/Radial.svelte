<script lang="ts">
	...
	import { BasicController, Radial } from '$lib/precisUI/lib/PrecisControllers';
...
>
	...
	<svg transform="scale(0.9)">
		...
		>
			<circle id="{dial.id}-circleFigure" fill="#112211" r={dial.width * (1 / dial.rx)} />
			<!-- circle ring   -->
			<circle
		...
		<!-- label -->
		<g id="{dial.id}-label">
			<text class="dial-label" x="50%" y="105%" dominant-baseline="middle" text-anchor="middle">
				{dial.label}
			</text>
		</g>
	</svg>
	<svg transform="scale(0.9)">
		...
		>
			...
			<!-- pointer plot -->
			<g id="pointerPlot" stroke-width="6">
				{#each pointerPlot.slice(4, 13) as { x, y }, i}
